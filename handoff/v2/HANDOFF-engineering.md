...
- **Backups:** **Litestream → Cloudflare R2**, prefix **`karel/`** (DB). Media backup is separate — see §5 / §6. Fresh builds restore the DB from R2.
...
- **M6 — Backups & ops hardening.** Verify Litestream fresh-build restore (DB). Configure media **object versioning + nightly mirror to R2 `karel-media/`** and run a **restore drill** (PRD §8/§11).
...
- **Backups — media:** DB is Litestream→R2 (`karel/`). **Media is separate:** enable **object versioning** on the media bucket + a **nightly `rclone sync` / MinIO replication** to R2 `karel-media/` (droplet cron or Coolify scheduled task; same R2 account, distinct prefix). Restore = re-provision bucket, `rclone sync` R2→bucket, repoint `MEDIA_PUBLIC_BASE_URL`. This can live at the ops/cron layer (env `MEDIA_BACKUP_*`) rather than in the Go app — either is fine, but it must exist and be drill-tested.
...
- **Backups:** DB fresh-build restore from R2 (`karel/`); **media restore drill** — versioning on + `rclone` mirror to `karel-media/`, re-sync into a fresh bucket, images return and `public_url`s resolve.
...
- **Ops prerequisites** (do before/with M0/M6): provision the Coolify S3 media bucket (+ versioning + R2 mirror), create the Turnstile keys, set up the Resend sender domain for `CONTACT_FROM`.

...
> Status: Draft (open questions resolved 2026-08-05; reconciled with delivered design 2026-08-07; media-backup spec added 2026-08-08) · Owner: Karel · Last updated: 2026-08-08
> Slug: `karel` · Repo: `ws-tilcer-karel` · Companion spec: `openapi.yaml` (OpenAPI 3.1, v0.1.0) — drafted 2026-08-05
> Design handoff: `HANDOFF-design.md` (**wow/gamified/summer** brief) — design delivered 2026-08-07 (**calm mode removed during design — see §10**)
...
- **Exposure:** public. The whole public site and its read APIs are public. The CMS admin area and all write endpoints sit behind the shared auth backend (**Mode B**, site id `karel`). Three public write endpoints (contact form, minigame score, link-click) are open by design and protected by anti-abuse guards.
- **Consumers:**
...
- **Playful and gamified**, with room for **several minigames**, delivered as a **single full-experience register** (the earlier calm/reduced-mode alternative was dropped during design — see §10). Motion-sensitive visitors are served by honoring **`prefers-reduced-motion`** (ambient animation/physics stilled); no piece of content lives *only* inside a game or animation.
...
- **No calm/reduced experience toggle** — removed during design (§10); motion safety is handled by `prefers-reduced-motion` alone.
...
Full detail in `openapi.yaml`. Backend served at `karel.tilcer.cz/api` (no strip-prefix, per the two-app deploy pattern). `kaja.tilcer.cz` 301s to `karel.tilcer.cz` at the edge (FR-2).
...
React + TypeScript + Vite SPA (static Nginx image), TanStack Query. **Bilingual CZ/EN** with an instant in-page switch (Czech default). Served only on the canonical `karel.tilcer.cz`. The CMS **admin area is the same SPA with a lazy-loaded authed `/admin`** bundle (kept out of the public bundle). **Design delivered 2026-08-07** (`HANDOFF-design.md` + the `*.dc.html` prototypes); this section is the record, reconciled with what was built.

### 7.1 Design direction — the brief that drove the design

> **The visual wow-effect is the single most important success factor of this project.** This site is Karel's personal brand front door and must be **genuinely stunning**, not a template. The design carries:
>
> - **Summer vibes** — a bright, joyful, warm palette and mood; **ice-cream** motifs (scoops, cones, sprinkles, melt/drip, popsicles), sun, beach/pool energy. Playful, tactile, delicious. (Delivered with a day + a "night" theme.)
> - **Gamified & interactive** — physics, hover/scroll surprises, easter eggs, a playful cursor, delightful micro-interactions, and **several minigames** woven into the experience. The `arcade` backend persists their scores; **game keys and score bounds are defined in code**.
> - **Wow on first paint** — a hero moment that makes visitors smile within the first second.
>
> **Motion safety (calm mode removed).** The brief originally required a "calm mode" dual-register; this was **dropped during design at Karel's request** — the site ships a **single full-experience register** (see §10). Motion-sensitivity is instead handled by **honoring `prefers-reduced-motion`**: ambient animation, hero physics, and decorative loops are stilled for those users, while functional motion (loaders, active gameplay) is preserved. The underlying principle still holds: **all content (projects, about, skills, links, legal info, contact) is reachable and legible without playing a game** — gamification enhances, it never gates content.

### 7.2 Public screens

- **Home / hero** — the wow moment; entry to everything; a persistent control cluster with the **language toggle + sound (mute) toggle** (no calm toggle).
...
- **404** — on-theme, playful, both languages, with a way home.

### 7.3 Admin (CMS) area

Behind Mode B auth at `/admin` (lazy-loaded within the same SPA), in a **deliberately quiet, function-first register** (distinct from the wow public site). Screens: projects (list/editor with bilingual fields + media picker + reorder), categories (CRUD + reorder), links (editor + reorder), sections (About/Hero), skills, business info, media library (upload/manage), contact inbox, arcade moderation. Bilingual editors flag missing translations (FR-15).
...
**Updated during design (2026-08-06 – 08-07):**
...
- [ ] **Motion safety:** `prefers-reduced-motion` stills ambient animation and the hero physics (functional motion preserved), and all content is reachable/legible without playing a game. (Calm mode was removed during design — §10; there is no calm toggle.)
- [ ] Design delivers a genuine **wow-effect** with summer/ice-cream theme and gamified interactions (verified against the delivered design).

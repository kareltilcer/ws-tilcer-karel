...
> For: **Claude Design** · Owner: Karel · Last updated: 2026-08-05 · Status: v1 brief
...
### The one hard constraint — calm mode (non-negotiable)

The site must be **fully usable and pleasant for visitors who do not want the gamification.** Confirmed with Karel:
...
- **A motion language** — the vocabulary of how things wobble/melt/bounce/drip — defined as reusable tokens (durations, easings, spring configs), *and* its calm-mode counterpart (what each motion collapses to when reduced).
...
Minigames are a **full-experience feature**: in calm mode the Arcade still *exists* as a section (don't 404 it), but present it as an opt-in ("Enable the fun to play") rather than auto-running animation.
...
- A **language toggle** that's always reachable (pairs naturally with the calm-mode toggle in a small persistent control cluster — propose the placement).
...
Design each screen in its **default, empty, loading, and error** states, **in both full-experience and calm-mode registers**, at **mobile (375 px) and desktop (1440 px)**, in **both languages** (verify the Czech length). Data field names come from `openapi.yaml`.

### 6.1 Home / hero — the wow moment
The make-or-break screen. This is where "whoa" happens. An immersive summer/ice-cream hero with the strongest interaction on the site, plus a clear path onward to everything (projects, about, linktree, arcade, contact). Must still communicate *who Karel is* in one glance for the goal-directed visitor. Design the **calm-mode hero** too — a beautiful, still, fast first impression.
...
### 6.7 Arcade
The minigame hub + the games + leaderboards + score submit (see §5). Full-experience feature; calm-mode shows an opt-in gate.
...
A footer with the Impressum link, language + calm-mode toggles (if not placed elsewhere), and light navigation. Plus the persistent **control cluster** (language, calm mode, optional sound mute) — design where it lives so it's always findable without fighting the hero.

### 6.11 404 / error
An on-theme, playful (in full mode) not-found page with a way home — in both languages.
...
- Function-first states: loading, empty, error, and the **show-once**/destructive confirmations (delete project/category/link, reset leaderboard, delete media). Keep it calm and quick.

## Component inventory

Public: the hero interaction; project card + category-filter chips; project-detail gallery/lightbox; linktree row/button; skill chip/meter; **minigame shells** (attract/playing/game-over) + **score-submit card** (name + Turnstile) + **leaderboard**; the persistent **control cluster** (language toggle, calm-mode toggle, sound mute); playful empty states + 404; contact form with Turnstile; footer. Define each with its **full vs calm** variant.
...
1. **The full ↔ calm duality.** One site, two coherent registers, an obvious always-reachable toggle, and `prefers-reduced-motion` respected. Calm mode must look *designed*, not disabled. This is the defining challenge — address it first and thread it through every screen.
...
3. **Delight for the wanderer *and* speed for the goal-directed visitor.** The person who came for one link or one project must reach it in seconds without sitting through animation; the wanderer must be rewarded for exploring. Both, at once.
...
- **`prefers-reduced-motion` is a first-class path**, not a nicety — it maps to calm mode. Nothing essential may depend on motion.
- **Motion-sensitivity safety:** no autoplaying audio; provide the calm toggle prominently; avoid strobe/rapid-flash; give any parallax/physics an off switch.
- **Performance:** the wow must not tank load. Budget the hero for a fast first paint; lazy-load games and heavy assets; keep the admin bundle out of the public bundle (it's lazy `/admin`). Target smooth on a mid-range phone; provide a reduced-fidelity path for low-power devices (tie it to calm mode / reduced-motion).
...
1. **Design-system doc** — tokens (summer color palette with **AA-verified** contrast; type scale incl. a Czech-diacritic-safe display + body pairing; spacing, radii, elevation; the **motion vocabulary** and its calm-mode collapse), the component inventory with **full vs calm** variants and all states, plus OG-image direction. Expressed against the build stack (React + TS + Tailwind; note any library beyond it).
2. **Hi-fi interactive prototype** — a self-contained single file showing the key public screens **in both full and calm registers**, at **375 px and 1440 px**, in **CZ and EN**, with at least one minigame demonstrated end-to-end (attract → play → game-over → submit → leaderboard). In-memory state, no backend. Make both breakpoints and both registers easy to view (toggles/frames).
...
- [ ] A hero/first-impression that delivers a genuine **wow**, on mobile and desktop, in full mode — and a lovely, fast **calm-mode** counterpart.
- [ ] **Calm mode + `prefers-reduced-motion`** fully designed: every screen reachable and legible, nothing essential behind motion/games, an obvious always-available toggle.
- [ ] All public screens (§6.1–6.11) in **default/empty/loading/error**, **full + calm**, **375/1440**, **CZ + EN**.
- [ ] The **Arcade**: hub + ≥2 minigames with attract/playing/game-over/submit(+Turnstile)/leaderboard, and the calm-mode opt-in gate.
...
- [ ] Summer/ice-cream identity tokenised; **AA contrast verified** in the palette; **Czech diacritics uncramped** at display size; realistic Czech content in the prototype (never lorem/English placeholders).
- [ ] All nine **hard problems** addressed with stated rationale.
...
1. **Daylight-only, or add an "evening/night" theme variant?** Daylight is the hero; is a dark summer-night mode worth the effort, and does it strengthen the concept?
2. **Sound design** — how far? A subtle opt-in layer, or richer game audio? (Off by default regardless.)
3. **The minigame roster** — which games, and how many, hit the sweet spot between "wow, several games" and scope? Propose the set + mechanics + score model.
4. **Navigation model** — single long scrolly page, a routed multi-page site, or a hybrid (scrolly hero → routed sections)? What best serves both the wanderer and the goal-directed visitor?
5. **Control-cluster placement** — where do the language + calm + mute toggles live so they're instantly findable without fighting the hero?
